...
#
# Purpose: Accepts Traditional Alphanumeric Code form of the Space Weather Advisory and generates equivalent IWXXM form.
#
...
class Encoder(E.Encoder):
    ...
    def __init__(self):
        ...
        self.re_AHL = re.compile(
            r'FN(?P<aaii>\w\w\d\d)\s+(?P<cccc>\w{4})\s+(?P<yygg>\d{6})(\s+(?P<bbb>[ACR]{2}[A-Z]))?'
        )  # noqa:501
